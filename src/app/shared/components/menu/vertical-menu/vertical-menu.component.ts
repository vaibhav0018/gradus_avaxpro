...
getUserMenuList(root_id : any): void {
  console.log('inside getUserMenuList ' );
  //alert('inside getUserMenuList');
  ...
  this.menuService.getUserMenuList({
    ...
    if (data.responseStatus === 'SUCCESS' && data.responseCode === 'RES_200') {
      ...
      data.responseData[0].map((item: any) => {
        ...
        if (item.level == 1) {
          if(item.json_data!=undefined && item.json_data!=null && item.json_data!='null'){
            ...
            aryLevel1['child'].forEach((element: any) => {
              ...
              if (objChild != undefined && objChild != null && (JSON.parse(objChild.json_data) != null)) {
                ...
                let sorted_ary = aryx.sort(function (a: any, b: any) {
                  return Number(a.serial_no) - Number(b.serial_no);
                });
                element.child.child = sorted_ary
              }
              else {
...
onClick(menu: any) {
  if (sessionStorage.getItem("recent_menu") != null) {
    let stateDataStr: any = sessionStorage.getItem("recent_menu");
    this.lstRecentMenu = JSON.parse(stateDataStr)
  }    
