...
export class ModifyHeaderPageComponent implements OnInit {
  ...
  getPartyDetail(party_code : any, flg : any, flag?: string) {  
      ...
      subscribe(data => {
        if (data.responseStatus === 'SUCCESS' && data.responseCode === 'RES_200') {
          if (flg == 'B') {
            ...
            if (flag == 'Y') {
              let addrBillToFirst = data.responseData[0].find(({ csad_addr_code  }: { csad_addr_code: any; }) => csad_addr_code == this.objDraftHeader.qt_bill_addr_code);
            ...
            if (flag == 'Y') {
              let addrDisToFirst = data.responseData[0].find(({ csad_addr_code } : { csad_addr_code: any; }) => csad_addr_code == this.objDraftHeader.qt_disp_addr_code);
  ...
  getHandledByDropdown() {
    ...
    this.utilityServiceAvaxPro.getHandledByList().subscribe({
      next:(data : any) => {
        ...
        let handledByObj = this.lstHandledBy.find(({ usr_userid } : { usr_userid: any; }) => usr_userid
          == this.objDraftHeader.qt_handled_by);
        this.form.controls.txtHandledBy.setValue(handledByObj);
        let followedByObj = this.lstHandledBy.find(({ usr_userid }  : { usr_userid: any; }) => usr_userid
          == this.objDraftHeader.qt_follow_up_by);
        this.form.controls.txtFollowedBy.setValue(followedByObj);
        let instByObj = this.lstHandledBy.find(({ usr_userid }  : { usr_userid: any; }) => usr_userid
  ...
  updateBillTo() {
    ...
    this.utilityServiceAvaxPro.checkParty(this.form.get('txtBillTo')?.value.cs_code+"::"+this.form.get('txtDispatchTo')?.value.cs_code).subscribe({
      next :(data : any) => {
